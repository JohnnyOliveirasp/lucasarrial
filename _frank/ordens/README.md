...
| 💸 **Estorno de treino (#469): corrigido e no ar — 1 branch STALE no origin, não mergear** | O `#469` (crédito criado do nada: N falhas na MESMA voz = N estornos contra 1 débito) foi **fechado em 24/09 15hZ**. O que vale em produção é **`aadddad3`** (18/09 16:37Z): `houveDebitoDeTreino` **removida** (0 ocorrências na árvore) e substituída por `saldoPendenteDoTreino` (`credits/service.ts`) + `valorDoEstornoDeTreino` (`credits/onboarding-cobranca.ts`), que decidem por **saldo pendente** (`Math.min(devido, teto)`, e `0` quando `saldoPendente >= 0`). Conferido no **conteúdo da `origin/main`** via `git cat-file`, ancestral confirmado, deploy SUCCESS. Controle de mutação: a perna do estorno casando por `kind` em vez de `ref_type='voice_train_refund'` derruba **1 de 5** em `saldo-pendente-do-treino.test.ts`; restaurada, 5/5. ⚠️ **Existe no origin o branch `fix/estorno-treino-por-saldo-pendente`** (HEAD `c9d3bc75`, **22/09** — era o **PR #398**, hoje CLOSED): é uma **segunda tentativa independente** pro MESMO defeito, escrita **4 dias DEPOIS** do conserto já estar no ar, justamente porque o cartão tinha ficado aberto. Ele **não conhece `saldoPendenteDoTreino`** (0 ocorrências) e mergear hoje **apaga `saldo-pendente-do-treino.test.ts` inteiro (−126 linhas)** — ou seja derruba o conserto E a guarda que prova que ele pega a regressão. **Mesma família** do `feat/onedrive-401`, do `feat/fix-image-upload-retry`, das 2 da cura de referência, do `fix/trava-foto-nova-8379549c` e do `fix/ritmo-da-referencia-porta-73a60bb` — e a **6ª** vez que a casa produz duas correções concorrentes pro mesmo bug. Não mergear; se for descartar de vez, apagar no origin. |
