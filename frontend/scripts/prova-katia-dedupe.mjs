...
import { classifyComplaint, incidentSignature } from "../src/lib/agent/mail-incident.ts";
...
console.log(
  `replay 20/08 17:05: o card de eco está gravado sob "${inc.signature}"; a queixa de pacing procura por "${sigPacing}" → não acha → abrirChamadoReportado() cria chamado NOVO, visível.`,
);
console.log(
  "se um card de pacing já existisse, a mesma chave somaria ocorrência nele (e reabriria, se estivesse fechado) — quem decide isso é lib/incidents/reportar.ts.",
);
