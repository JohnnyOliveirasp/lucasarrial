...
async function renderProject(projectId, userId) {
  const { data: project } = await supabase
    .from("video_projects")
    .select("id, audio_path, script_text, subtitle_style, subtitle_position, subtitle_size")
    .eq("id", projectId)
    .maybeSingle();
  ...
  const { data: scenes } = await supabase
    .from("video_scenes")
    .select("idx, video_path, video_status")
  ...
  try {
    ...
    await supabase
      .from("video_projects")
      .update({ final_video_path: key, status: "done", error_message: null })
      .eq("id", projectId);
...
// ── Loop da fila ──────────────────────────────────────────────────────────
async function tick() {
  const { data: job, error } = await supabase.rpc("claim_render_job");
  if (error) {
    console.error("[worker] claim erro:", error.message);
    return;
  }
  ...
  try {
    ...
    await supabase.from("render_jobs").update({ status: "done", updated_at: new Date().toISOString() }).eq("id", job.id);
    console.log(`[worker] ✅ job ${job.id} pronto → ${key}`);
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    console.error(`[worker] ❌ job ${job.id} falhou:`, msg);
    await supabase.from("render_jobs").update({ status: "failed", error: msg.slice(0, 800), updated_at: new Date().toISOString() }).eq("id", job.id);
    await supabase.from("video_projects").update({ status: "failed", error_message: msg.slice(0, 500) }).eq("id", job.video_project_id);
  }
}

async function main() {
  if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
    console.error("Faltam envs do Supabase. Rode com: node --env-file=.env.local render/worker.mjs");
    process.exit(1);
  }
