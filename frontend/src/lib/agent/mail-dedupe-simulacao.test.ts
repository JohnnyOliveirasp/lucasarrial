...
try {
  ...
  mock.module("./mail-imap", {
    namedExports: {
      ...
      // `fioDoAluno` (histórico do fio) entrou na main DEPOIS que este teste foi
      // escrito e importa `fetchThread` daqui. Sem este export a simulação
      // inteira virava SKIP — e SKIP sai com exit 0, que é pior do que falhar:
      // parece verde e não provou nada. Fio vazio é o próprio fallback
      // best-effort do mail-respond, então não altera o que se está medindo.
      fetchThread: async () => [],
    },
  });
