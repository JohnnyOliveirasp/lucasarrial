...
export function StudioWorkspace({
  ...
  async function startMontage(musicKey: string | null, captions: boolean, editStyle: "dynamic" | "sober") {
    if (!project) return;
    setBusy("submit");
    setError(null);
    try {
      const res = await fetch(`/api/v1/studio/${project.id}/montage`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ music_key: musicKey, captions, edit_style: editStyle }),
      });
      const j = await res.json().catch(() => ({}));
