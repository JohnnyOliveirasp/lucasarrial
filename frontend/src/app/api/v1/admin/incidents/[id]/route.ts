...
export async function PATCH(
  ...
  // "ignored" também é fechamento: sem resolved_at o incidente fica invisível
  // pro detector de zumbi (bug medido em 20/08 — só "fixed" gravava a data).
  const update: Record<string, unknown> = {
    status,
    ...closureFields(status, g.auth.email ?? "admin (sem e-mail no gate)"),
  };
